...
- `ios-profile.json` — the iOS `Profile` Codable JSON (camelCase). iOS now
  models `middle_name`, `street_address_2` and the EEO block, so they appear
  here and round-trip through the rename map.
...
- `canonical_to_ios` equals `ios-profile.json` (rename map; iOS now models the
  full scalar set).
- Base-overlay round-trips both directions with **zero field loss**: a desktop
  import keeps its local secrets, and any future canonical-only key iOS doesn't
  model yet survives an iOS edit via the base overlay.
