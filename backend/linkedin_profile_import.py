...
_SECTIONS = (
    ...
    ("PROFILE OVERVIEW", "", 5500),
    ("ALL EXPERIENCE", "details/experience/", 6500),
    ("ALL EDUCATION", "details/education/", 1500),
    ("LICENSES & CERTIFICATIONS", "details/certifications/", 1200),
    ("SKILLS", "details/skills/", 800),
)
...
endorsement counts.
- "certifications": name each certification once, optionally with issuer.
...
async def _page_text(page) -> str:
    """Scroll to trigger lazy loading, then grab the main content's text."""
    for _ in range(4):
        try:
        ...
        for label, suffix, _cap in _SECTIONS:
            ...
            try:
                ...
                if "/login" in page.url or "/authwall" in page.url:
                    ...
                    continue
                sections.append((label, await _page_text(page)))
